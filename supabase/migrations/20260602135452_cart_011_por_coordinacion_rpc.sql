-- CART-011: RPC cartera_por_coordinacion(p_fecha_corte date) returns json.
-- Devuelve un array de coordinaciones con sus totales y distribución PAR (8 buckets).
-- Mismo contrato visual que cartera_resumen pero "explotado" por coordinacion,
-- pensado para alimentar la tabla coord × PAR del dashboard /cartera/coordinacion.

create or replace function public.cartera_por_coordinacion(p_fecha_corte date)
returns json
language plpgsql
stable
security definer
...
begin
  ...
  totales_coord as (
    ...
    from base
    group by coordinacion
  ),
  -- Producto cartesiano coord × bucket para garantizar todos los buckets en todas las coords.
  coord_bucket as (
    select c.coordinacion, d.bucket, d.label, d.dias_min, d.dias_max, d.orden
    from (select distinct coordinacion from base) c
  ...
  par_por_coord as (
    ...
    from coord_bucket cb
    left join clasificado cl
      on cl.coordinacion = cb.coordinacion
     and cl.bucket = cb.bucket
    group by cb.coordinacion, cb.bucket, cb.label, cb.dias_min, cb.dias_max, cb.orden
  ),
  -- Arma el array de buckets por coordinación (ordenado).
  par_agg as (
    select
      coordinacion,
  ...
  -- Une totales + par + calcula porcentajes.
  por_coord as (
    select
      t.coordinacion,
      t.creditos,
      ...
      p.par
    from totales_coord t
    left join par_agg p on p.coordinacion = t.coordinacion
  )
...
revoke all on function public.cartera_por_coordinacion(date) from public;
revoke all on function public.cartera_por_coordinacion(date) from anon;
grant execute on function public.cartera_por_coordinacion(date) to authenticated;

comment on function public.cartera_por_coordinacion(date) is
  'CART-011: cartera × PAR agregada por coordinacion. Para cada coordinación devuelve totales (creditos, cartera_total, en_mora, pct_mora) + indicadores (pct_par_30/90) + array par con 8 buckets. Ordenado por pct_par_30 desc (peor región arriba). Métrica = saldo_total. Security definer + check rol=admin OR acceso_cartera=true.';
