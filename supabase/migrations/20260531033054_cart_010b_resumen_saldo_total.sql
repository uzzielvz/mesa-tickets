-- CART-010b: ajuste de métrica en cartera_resumen.
-- Cambio: la métrica monetaria pasa de `saldo_riesgo_total` a `saldo_total`.
-- Razón: `saldo_riesgo_total` infla los porcentajes PAR (denominador casi 0 para créditos
-- corrientes). Estándar industria PAR = saldo principal pendiente / saldo principal total.
-- Validación 2026-05-30 con 215 filas (fecha_corte 2026-05-30):
--   PAR>30 actual=67.41%  → con saldo_total=34.52%  (más interpretable)
--   PAR>90 actual=31.16%  → con saldo_total=15.95%
-- Contrato del JSON resultante NO cambia, solo los valores.
...
begin
  ...
  select coalesce(sum(saldo_total), 0) into v_total_cartera
...
comment on function public.cartera_resumen(date) is
  'CART-010: snapshot ejecutivo de cartera para una fecha_corte. Métrica = saldo_total (no saldo_riesgo_total). Devuelve totales, distribución PAR (8 buckets) y porcentajes PAR>30/PAR>90. Valida acceso (admin o profiles.acceso_cartera=true).';
